...
contract Exchange is ERC20 {
    ...
    // Returns the amount `Crypto Dev Tokens` held by the contract
    function getReserve() public view returns (uint) {
        return ERC20(cryptoDevTokenAddress).balanceOf(address(this));
    }

    // Adds liquidty to the exchange
    ...
    function removeLiquidity(uint _amount) public returns (uint , uint) {
        ...
        // Burn the sent LP tokens from the user's waller because they are already sent to remove Liquidty.
    ...
    function getAmountOfTokens(uint256 inputAmount,uint256 inputReserve,
